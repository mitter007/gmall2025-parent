...
import com.atguigu.common.TableProcess;
...
public class Metadata implements Closeable {
    ...
    private Map<String,String> eventIpNatMap = new HashMap<String,String>(){{
        put("hadoop202","hadoop205");
        put("hadoop203","hadoop206");
        put("hadoop204","hadoop207");
    }};
    ...
    private MetadataCache handleMetadataResponse(MetadataResponse metadataResponse, boolean isPartialUpdate, long nowMs) {
        ...
        // 添加如下代码
        // 原始是不可修改的 map
        Map<Integer, Node> originalMap = metadataResponse.brokersById();
        Map<Integer, Node> brokersById =new HashMap<>(originalMap);
...
//        Map<Integer, Node> nodes = metadataResponse.brokersById();
        if (isPartialUpdate)
            return this.cache.mergeWith(metadataResponse.clusterId(), brokersById, partitions,
                unauthorizedTopics, invalidTopics, internalTopics, metadataResponse.controller(), topicIds,
                (topic, isInternal) -> !topics.contains(topic) && retainTopic(topic, isInternal, nowMs));
        else
            return new MetadataCache(metadataResponse.clusterId(), brokersById, partitions,
                unauthorizedTopics, invalidTopics, internalTopics, metadataResponse.controller(), topicIds);
    }
